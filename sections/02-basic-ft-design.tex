...
对于一个希望提供fault tolerance能力的VM（primary VM），%
...
primary VM收到的所有输入都会通过一条称为logging channel的%
网络连接发送给backup VM。%
对于服务器的工作负载而言，主要的输入流量来自网络和磁盘。如下文section 2.1所述，%
为了确保backup VM与primary VM以相同的方式执行%
non-deterministic operation，必要时还会传输额外信息
\emph{(这里后续会提到，backup不接收自己机器上的timer中断，也不会%
收到来自外界的报文，而是通过logging channel稍带这些信息，使得%
它们被插入到和primary VM相同的执行点，以消除状态偏移)}。%
...
primary服务器和backup彼此失去通信的这种脑裂情况，%
\emph{(译者注：常见的一种脑裂情况是两台服务器分区，彼此%
无法通讯，后面两台机器都认为自己是primary)}%
...
控制中断。此时hypervisor可以统一处理本epoch内缓存起来的%
普通VM中断。这里的普通VM中断，是指timer interrupt、I/O完成%
interrupt、network interrupt等guestOS正常运行时会看到的中断，%
它们不同于用于结束epoch的控制中断。%
...
这也是为什么这个系统能较少地影响机器性能的部分原因)}。由于操作系统使用非阻塞的网%
...
\\\emph{译者注：论文中的shared storage不是由guestOS%
直接访问的网络盘，也不是挂载在guestOS文件系统层中的%
对象。它通常是hypervisor层可以共同访问%
的共享存储，例如SAN、iSCSI、FC或NFS datastore。%
VM的virtual disk存放在该shared storage上，%
hypervisor再把这个virtual disk模拟成一块普通磁%
盘设备呈现给guestOS。guestOS看到的是类似/dev/sda%
这样的本地块设备，并在其上挂载自己的文件系统；但真正%
把虚拟磁盘I/O映射到sharedstorage读写的是%
hypervisor/VMkernel，而不是guestOS的网络栈。
\\可见test-and-set的操作也是guestOS无感知的，%
完全由hypervisor层进行。}%
