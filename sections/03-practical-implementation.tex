...
或时间同步机制，让guestOS在下一次读取clocksource或调用%
...
例如timer interrupt、network input、disk I/O completion的信息)}%

如果backup VM在需要读取下一条log entry时遇到空的log buffer%
，它会停止执行，直到有新的log entry可用。由于backup VM不与外部通信%
...
\\\emph{译者注：下面是简化的primary执行与日志生成伪代码；实际实现还包含%
...
或 8GB，guest OS也会认为自己拥有这么多“物理内存”。但是这%
只是guest看到的guest physical memory大小，这些guest物理页背%
后仍然需要宿主机(host)真实物理内存承载。如果承载backup VM 的机器上%
...
但如果它之后再次访问这些被压缩或换出的页面，就需要等待 hypervisor%
...
却可能因为host层面的物理内存竞争、换页、向其他 VM 回收页面，%
...
\\\emph{译者注：\textbf{Output Rule}会把对外输出额外延迟到backup VM ACK%
对应log entry之后。在游戏服务器中，这个延迟尤其影响用户体验，假如ack时间是40ms%
那么游戏服务器的出包延迟很可能就会多出接近40ms，有的游戏玩家根本接受不了这种延迟。}%
...
阻塞的，因此可以并行执行\emph{(译者注：现代网卡发送通常也采用类似的异步%
提交模型：驱动填写descriptor并通知设备，设备通过DMA处理数据，随后报告完成)}，%
...
primary可能读到的是老数据，也可能是从磁盘数据读出来的数据，甚至由于我们没有%
等待中断，可能是混杂数据。而backup由于通常落后primary一段时间，等它replay%
到读取A的指令时，很明显，它读的就是A内存的老数据，不可能是磁盘数据或者混杂数据。%
这样primary和backup在同一条读取指令上就可能读到不同的值，从而导致VM状态分叉。%
正常驱动通常会等待completion后才把数据当作有效结果使用，但错误驱动、恶意guest%
或其他并发内存访问仍可能触发这种竞争，因此hypervisor必须保证行为确定。%
...
记录下此刻的CPU计数器用来生成log entry发给log buffer，随即恢复VM执行。%
...
\\\\磁盘I/O则不同。guest OS把本地磁盘看作块设备，请求发出后通常期待一个%
